...
*/
class BURDShell_Ubuntu extends BURDShell_interface {
	...
	{
	    exec("sudo ls -l /etc/apache2/sites-available/", $out_lines);	    
		$this->print_output($out_lines);	 		
	}
		...
		else 
		{
			...
			else
			{
				...
				else 
				{
					...
					if (!file_exists("/etc/apache2/sites-available/".$user_input)) 
					{
					...
					else
					{
						...
						//Delete config
						exec("sudo rm /etc/apache2/sites-available/".$user_input, $out_lines);
						$this->print_output($out_lines);	 		
			...
			else
			{
				...
				else 
				{
					...
					if (!file_exists("/etc/apache2/sites-available/".$user_input)) 
					{
						...
						// Save file
						file_put_contents("/etc/apache2/sites-available/".$user_input, $template);
		...
		else 
		{
			...
			else
			{
				...
				else 
				{
					...
					{
					    exec("svnlook history ".Config::$shell_folder."/svn/".$user_input." --show-ids", $out_lines);	   
						$this->print_output($out_lines);	 
					}
				}
			}
			...
			else
			{
				...
				else 
				{
					...
					else
					{
						...
						if (empty($revision))
						{
						    exec("svnlook youngest ".Config::$shell_folder."/svn/".$user_input, $out_lines);	   
					    ...
					    exec("svnlook log".$revision." ".Config::$shell_folder."/svn/".$user_input, $out_lines);	   
						$this->print_output($out_lines);	 
			...
			else
			{
				...
				else 
				{
					...
					else
					{
						...
						exec("svnlook changed ".Config::$shell_folder."/svn/".$user_input." --revision ".$revision, $out_lines);	   
						if (count($out_lines))
					    {
						...
						$out_lines = array();
					    exec("svnlook log ".Config::$shell_folder."/svn/".$user_input." --revision ".$revision, $out_lines);	   
			...
			else
			{
				...
				else 
				{
					...
					else
					{
					    ...
					    exec("svnadmin dump -q ".Config::$shell_folder."/svn/".$user_input." | gzip -9 > ".Config::$backup_folder."/bk_svn_".$user_input.".gz", $out_lines);	   
						$this->print_output($out_lines);	 
		...
		else 
		{
			...
			else
			{
				...
				else 
				{
						...
						// Dump database
					    exec("mysqldump -u root -p".$pass." ".$user_input." > ".Config::$backup_folder."/bk_database_".$user_input.".sql", $out_lines);	   
						$this->print_output($out_lines);	 
			...
			else
			{
				...
				else 
				{
					...
					else
					{
					    ...
					    exec("cat ".Config::$backup_folder."/bk_database_".$user_input.".sql | mysql -u root -p".$pass." ".$user_input, $out_lines);	    
						$this->print_output($out_lines);						
...
//!TODO
	...
	public function database_create()
	{
		...
		else 
		{
			...
			else
			{
				...
				{	
				    exec("mysqladmin -u root -p".Config::$db_admin_pass." create ".$user_input, $out_lines);	    
					$this->print_output($out_lines);						
				}
			}
		}		
	...
	public function database_delete()
	{
		...
		else 
		{
			...
			else
			{
				...
				{	
				    exec("mysqladmin -u root -p".Config::$db_admin_pass." drop ".$user_input." -f", $out_lines);	  
	...
	public function database_list()
	{
	    ...
	    exec("echo \"SHOW DATABASES;\" | mysql -u root -p".$pass." -t", $out_lines);	    
		$this->print_output($out_lines);						
	}	
	...
	public function database_security()
	{
		...
		else 
		{
			...
			else
			{
				...
				{
					if ($this->config['db_admin_pass']) 
					{
						$pass = $this->config['db_admin_pass'];
					}
					else
					{
						$pass = "";
